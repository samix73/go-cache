...
func BenchmarkCacheSetOptions(b *testing.B) {
	...
	benchmarks := []struct {
		...
		},
		{
			...
			new: func() *Cache[int, []byte] {
				return NewCache(WithEvictionStrategy[int, []byte](NewLRUEvictionStrategy[int](4096)))
			},
		},
	}
...
func BenchmarkCacheGetOptions(b *testing.B) {
	...
	benchmarks := []struct {
		...
		},
		{
			name: "max size with eviction",
			new: func() *Cache[int, []byte] {
				return NewCache[int, []byte](WithEvictionStrategy[int, []byte](NewLRUEvictionStrategy[int](4096)))
			},
		},
	}
